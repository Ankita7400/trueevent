...
<tr>
 <td>{{ $loop->iteration }}</td>
                    ...
                    <td>{{ $contact->event }}</td>
                    <td>{{ \Carbon\Carbon::parse($contact->eventdate)->format('d-m-Y') }}</td>
                ...
                {{-- ✅ ACTION BUTTONS --}}
                <td>
                    {{-- <a href="{{ route('admin.contactShow', $contact->id) }}" class="btn btn-sm btn-info">View</a> --}}

                    <form action="{{ route('admin.contact.destroy', $contact->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this?');">
                        @csrf
                        @method('DELETE')
